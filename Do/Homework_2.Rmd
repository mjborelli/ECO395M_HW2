...
- Accuracy = `r round(100*(sum(diag(baseline)/n)), digits = 2)`%
- Sensitivity = `r round(100*baseline[2,2] / (sum(baseline[2,])), digits=2)`%
- Specificity = `r round(100*(1 - (baseline[1,2] / sum(baseline[1,]))), digits = 2)`%
- Positive Predictive Value = `r round(100*(1 - (baseline[1, 2] / sum(baseline[, 2]))), digits = 2)`
...
p0 + geom_bar(stat = "identity", aes(x = Symptoms, y = V2)) + 
     labs(x = "Previous Symptoms", y = "Pct. Diagnosed w/ Breast Cancer")
```
...
p0 + geom_bar(stat = "identity", aes(x = V1, y = V2)) + 
     labs(x = "Menopause Status", y = "Pct. Diagnosed w/ Breast Cancer")
```
...
density_cancer = data.frame(Age=levels(density_cancer_setup$density),
                           stringsAsFactors = FALSE)
for (i in 1:4) {
